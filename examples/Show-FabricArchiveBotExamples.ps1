...
Write-Host "`n4. Parallel Processing (Default Behavior - PowerShell 7+):" -ForegroundColor Cyan
Write-Host "   .\Start-FabricArchiveBot.ps1" -ForegroundColor White
Write-Host "   (Parallel processing is enabled by default)" -ForegroundColor Gray

Write-Host "`n5. Custom Throttle Limit for Parallel Processing:" -ForegroundColor Cyan
Write-Host "   .\Start-FabricArchiveBot.ps1 -ThrottleLimit 4" -ForegroundColor White

Write-Host "`n6. Disable Parallel Processing (Serial Processing):" -ForegroundColor Cyan
Write-Host "   .\Start-FabricArchiveBot.ps1 -SerialProcessing" -ForegroundColor White
Write-Host "   (For compatibility or debugging purposes)" -ForegroundColor Gray

Write-Host "`n7. Custom Workspace Filter:" -ForegroundColor Cyan
Write-Host "   .\Start-FabricArchiveBot.ps1 -WorkspaceFilter `"(state eq 'Active') and contains(name,'Production')`"" -ForegroundColor White

Write-Host "`n8. Custom Target Folder:" -ForegroundColor Cyan
Write-Host "   .\Start-FabricArchiveBot.ps1 -TargetFolder 'C:\FabricBackups'" -ForegroundColor White

Write-Host "`n9. Skip Legacy Fallback (FabricTools Only):" -ForegroundColor Cyan
Write-Host "   .\Start-FabricArchiveBot.ps1 -SkipLegacyFallback" -ForegroundColor White

Write-Host "`n10. Migration from v1.0 to v2.0:" -ForegroundColor Cyan
Write-Host "   .\helpers\ConvertTo-FabricArchiveBotV2.ps1 -TestMigration" -ForegroundColor White

Write-Host "`n11. Full Production Example:" -ForegroundColor Cyan
Write-Host "   .\Start-FabricArchiveBot.ps1 -ConfigPath '.\FabricArchiveBot_Config-Production.json' -ThrottleLimit 6 -TargetFolder 'D:\FabricArchives'" -ForegroundColor White
